...
### v1.1.1
- Claudeの自動入力処理を大幅改善
  - ProseMirrorエディタでの入力安定性を向上（文字消失・送信ボタン無効化問題を解決）
  - Selection APIとexecCommand使用によりユーザー入力と同等の挙動を実現
- Manusの送信ボタン選択精度を向上
  - 添付ファイルボタンとの誤認識を防ぐため、より具体的なセレクタに改善
- 各AIサービスの自動送信処理の信頼性向上

### v1.1.0
- Claude（Anthropic社AI）を新たな送信先サービスとして追加
- Claude含む全AIサービスで自動入力・送信の安定性を向上
- 細かなバグ修正・UI改善
